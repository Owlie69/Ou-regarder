...
export interface VisibilityAnalysis {
  ...
  /**
   * When present, the map loads these pregenerated static assets instead of computing live.
   * Generated by running scripts/generate_eclipse_map.py (or a per-event equivalent).
   * Paths are relative to public/ (no leading slash).
   */
  pregeneratedAssets?: {
    zonesPath:   string   // FeatureCollection GeoJSON of visible zones (green = can see event)
    heatmapPath?: string  // PNG overlay (colour-coded by minutes of visibility)
    boundsPath?:  string  // JSON { bounds4326: [W,S,E,N] } giving heatmap PNG corners
  }
}
